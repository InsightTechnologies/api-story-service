...
public class StoryController extends APIMicroService {
	...
	public FeatureStoryDetails runService(@RequestBody FeatureMetadetails featureMetadetails) throws Exception {
		...
		try
		{
			...
			logger.error("Getting exception in extracting stories from feature, Exception Description :: "+storyException.getMessage(),storyException);
			throw storyException;
		}catch(GatewayServiceException gatewayServiceException){
			logger.error("Getting exception in extracting stories from feature, Exception Description :: "+gatewayServiceException.getMessage(),gatewayServiceException);
			throw gatewayServiceException;
		}
		catch(Exception exception) {
			logger.error("Getting exception in extracting stories from feature, Exception Description :: "+exception.getMessage(),exception);
			throw new StoryException("Getting exception in extracting stories from feature, Exception Description :: "+exception.getMessage(),
	...
	private String buildURLEndpointToGetFeature(FeatureMetadetails featureMetadetails) throws StoryException
	{
		...
		if(projectName !=null && !projectName.trim().isEmpty())
		{
			if(featuredID>0)
			{
				urlEndpoint.append(iceScrumURLPrefix).append(featureMetadetails.getProjectName()).append("/feature/").append(featuredID);
			}else {
